...
from src.units.subcharacters.LivelyStudent import LivelyStudent
...
class Personas:
    ...
    rory=LazyStudent("Rory", 10, 10, 10, 5, 5, Sprites.guy_tall_slender_backpack)

    lin=Genius("Lin",10,10,10,4,7,Sprites.girl_black_hair)

    jen = LivelyStudent("Jen", 10, 10, 10, 5, 6, Sprites.girl_brown_hair)

    chris=Brute("Chris", 10, 10, 10, 6, 4, Sprites.male_muscular_shirtless_behind)

    enemy1=Character("Wendie",10,200,200,5,5,Sprites.emoji_on_fire,600,200)

    enemy2=Character("Evil",10,145,100,10,10,Sprites.emoji_chill)

    math_professor_a=MathProfessor("Math Professor", 10, 200, 100, 5, 6, Sprites.professor_a)

    cs_professor_a=CSProfessor("CS Professor", 10, 200, 100, 5, 6, Sprites.professor_female_a)

    professor_b = Character("Professor B", 10, 200, 100, 5, 5, Sprites.professor_b)

    teaching_assistant_a = TeachingAssistant("Teach Assist A",10,150,150,4,4,Sprites.teaching_assistant_a)

    nu_husky_a= Husky("NU Husky",10,100,100,5,5,Sprites.nu_husky)
